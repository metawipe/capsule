...
interface GiftListItemProps {
  ...
  onBuy: (gift: GiftPreview) => void
}

function TonIcon() {
  return (
    <svg width="12" height="12" viewBox="0 0 16 16" fill="none" aria-hidden="true" className="gift-card__tonIcon">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
...
export const GiftListItem = memo(function GiftListItem({ gift, onCardClick, onBuy }: GiftListItemProps) {
  const triggerRef = useRef<HTMLDivElement>(null)
  const [visible, setVisible] = useState(!isLottiePreview(gift.preview))
  const isLottie = isLottiePreview(gift.preview)
  const { containerRef } = useLottie(gift.preview, visible && isLottie, true)
  ...
  return (
    <article className="gift-list-item" onClick={handleCardClick}>
      <div className="gift-list-item__thumb" ref={triggerRef}>
        {gift.preview ? (
          isLottie ? (
            <div ref={containerRef} className="gift-list-item__lottie" />
          ) : (
            <img src={gift.preview} alt="" loading="lazy" decoding="async" />
          )
        ) : (
          <span className="gift-list-item__thumbPlaceholder mdi mdi-gift" aria-hidden="true" />
        )}
      </div>

      <div className="gift-list-item__content">
        <div className="gift-list-item__info">
          <h3 className="gift-list-item__title">{gift.name}</h3>
          <p className="gift-list-item__id">#{gift.id}</p>
        </div>

        <div className="gift-list-item__actions">
          <button type="button" className="gift-card__price-btn gift-list-item__priceBtn" onClick={handleBuy}>
            <span className="gift-card__price">{gift.price.toFixed(2)}</span>
            <TonIcon />
          </button>
          <button type="button" className="gift-card__cart-btn" onClick={handleBuy} aria-label="Add to cart">
